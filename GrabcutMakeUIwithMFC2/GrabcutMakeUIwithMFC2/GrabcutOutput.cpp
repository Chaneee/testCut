	...
	for (int i = 0;i < originMat.cols;i++)
	{
		for (int j = 0;j < originMat.rows;j++)
		{
			...
			else if(canny.at<uchar>(j, i) == 255)
			{
				...
				GrabCutImg.at<Vec3b>(j, i)[0] = (BGMat.at<Vec3b>(j + target_Y, i + target_X)[0] + originMat.at<Vec3b>(j, i)[0]) / 2;

				GrabCutImg.at<Vec3b>(j, i)[1] = (BGMat.at<Vec3b>(j + target_Y, i + target_X)[1] + originMat.at<Vec3b>(j, i)[1]) / 2;

				//GrabCutImg.at<Vec3b>(j, i)[1] = /*BGMat.at<Vec3b>(j + target_Y, i + target_X)[1] / 200 **/ originMat.at<Vec3b>(j, i)[1];
				GrabCutImg.at<Vec3b>(j, i)[2] = (BGMat.at<Vec3b>(j + target_Y, i + target_X)[2] + originMat.at<Vec3b>(j, i)[2]) / 2;
...
vector <int> edgeX;
vector <int> edgeY;
void GrabcutOutput::EdgePosition()
{
	for (int i = 0;i < originGrabImg.cols;i++)
	{
		for (int j = 0;j < originGrabImg.rows;j++)
		{
...
void GrabcutOutput::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (isLeftMouseDown)
	{
		...
		RedrawWindow();
		//cv::addWeighted(BGimg, 1.0, GrabCutImg, 0.3, 0., BGimg);
