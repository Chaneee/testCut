...
void CDlgTab2::DisplayOutput(int IDC_PICTURE_TARGET, Mat targetMat)
{
	if (targetMat.cols > 956)
		cv::resize(targetMat, targetMat, cv::Size(956, targetMat.rows * (1 - (targetMat.cols - 956) / targetMat.cols)), 0, 0, CV_INTER_NN);
	if (targetMat.rows > 700)
		cv::resize(targetMat, targetMat, cv::Size(targetMat.cols * (1 - (targetMat.rows - 700) / targetMat.rows), 700), 0, 0, CV_INTER_NN);
...
//심카빙 실행
int CDlgTab2::seamcarving(int startPoint, Mat& sobelMat, int mode, int cutCount)
{
		...
		for (int startY = 0; startY < BGimg.rows; startY++)
		{
			...
			else
			{
				...
				for (int y = -1; y <= 1; y++)
				{
					...
					minTemp = findMin(ev[(startY + y) * BGimg.cols + (startPoint-1)], minTemp);
					if (minTemp == ev[(startY + y) * BGimg.cols + (startPoint-1)])	tmpY = y + startY;
				}

				ev[tmpY * BGimg.cols + startPoint] += minTemp;
				ret += ev[tmpY * BGimg.cols + startPoint];
				//startX = tmpX;
			}
		}
	}
		...
		for (int y = BGimg.rows - 1; y > 0; y--)
		{
			...
			else
			{
				...
				for (int x = -1; x <= 1; x++)
				{
					...
					minTemp = findMin(ev[(y - 1) * BGimg.cols + startPoint + x], minTemp);
					if (minTemp == ev[(y - 1) * BGimg.cols + startPoint + x])	tmpX = x + startPoint;
				}

				//ret += minTemp;
				startPoint = tmpX;
			}
			BGimg.at<Vec3b>(y - 1, startPoint)[0] = 0;
			BGimg.at<Vec3b>(y - 1, startPoint)[1] = 0;
			BGimg.at<Vec3b>(y - 1, startPoint)[2] = 255;
...
void CDlgTab2::OnBnClickedCallbgd()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		//988 - 32
		if(BGimg.cols > 956)
			cv::resize(BGimg, BGimg, cv::Size(956, BGimg.rows * (1 - (BGimg.cols - 956) / BGimg.cols)), 0, 0, CV_INTER_NN);
		if (BGimg.rows > 700)
		...
		for (int y = 1; y < BGimg.rows; y++)	seamcarving(y, sobelMat, 0, 0);
